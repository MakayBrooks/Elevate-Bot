...
async function sendJournalPanel(channel) {
  ...
  const logEmbed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      '> Click **Submit Trade of the Week** to showcase your best trade.\n\u200b'
    )
    .addFields(
  ...
  const row = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('journal_submit_totw').setLabel('\u{1F3C6} Submit Trade of the Week').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('journal_my_journal').setLabel('\u{1F4DA} My Journal').setStyle(ButtonStyle.Secondary),
  );

  // If a panel already exists, refresh its contents in place instead of reposting
  if (db._panelMessageId) {
    try {
      const existing = await channel.messages.fetch(db._panelMessageId);
      if (existing) {
        await existing.edit({ embeds: [logEmbed], components: [row] });
        return existing;
      }
    } catch {} // message deleted, fall through to fresh post
  }
...
function buildTotwManualModal() {
  return new ModalBuilder()
    .setCustomId('journal_totw_modal')
    .setTitle('\u{1F3C6} Trade of the Week')
    .addComponents(
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('totw_pair')
          .setLabel('Pair (e.g. NQ, ES, EURUSD)')
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
          .setPlaceholder('NQ')
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('totw_pnl')
          .setLabel('PnL (e.g. +$1,250)')
          .setStyle(TextInputStyle.Short)
          .setRequired(true)
      ...
      new ActionRowBuilder().addComponents(
        new TextInputBuilder()
          .setCustomId('totw_proof')
          .setLabel('Screenshot proof / trade summary')
          .setStyle(TextInputStyle.Paragraph)
          .setRequired(true)
          .setPlaceholder('Paste image link and/or describe the trade')
      )
    );
}
...
async function handleJournalInteraction(interaction, client) {
  ...
  // ── Submit Trade of the Week – entry point ───────────────────────────────
  if (interaction.isButton() && interaction.customId === 'journal_submit_totw') {
    try {
      const db = loadDB();
      const trades = db[interaction.user.id]?.trades || [];
      if (!trades.length) {
        await interaction.showModal(buildTotwManualModal());
        return;
      }
      const recent = trades.slice(-25).reverse();
      const selectMenu = new StringSelectMenuBuilder()
        .setCustomId('journal_totw_select')
        .setPlaceholder('🏆 Pick one of your journaled trades...')
        .addOptions(recent.map((t, i) => {
          const globalIdx = trades.length - 1 - i;
          const outcome = (t.outcome || '').trim().toLowerCase();
      ...
      const row2 = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('journal_totw_manual').setLabel('✍️ Enter a Different Trade').setStyle(ButtonStyle.Secondary),
      );
      await interaction.reply({
        content: '🏆 **Submit Trade of the Week** — pick a journaled trade or enter one manually.',
  ...
  if (interaction.isButton() && interaction.customId === 'journal_totw_manual') {
    try {
      await interaction.showModal(buildTotwManualModal());
    } catch (err) {
      if (err.code === 10062 || err.rawError?.message?.includes('already been acknowledged')) return;
  ...
  if (interaction.isStringSelectMenu() && interaction.customId === 'journal_totw_select') {
    await interaction.deferUpdate();
    try {
      const tradeIdx = parseInt(interaction.values[0], 10);
      const db = loadDB();
      const trade = db[interaction.user.id]?.trades?.[tradeIdx];
      if (!trade) { await interaction.editReply({ content: '❌ Trade not found.', components: [] }); return; }
      ...
      if (adminChannel) {
        ...
        const embed = new EmbedBuilder()
          .setColor(color)
          .setTitle('🏆 Trade of the Week Submission')
          .setDescription(`**${interaction.user.username}** submitted Trade #${tradeIdx + 1} — 📓 *linked from journal*.`)
          .addFields(
            { name: '👤 User', value: `${interaction.user}`, inline: true },
            { name: '📅 Date', value: trade.datetime || 'N/A', inline: true },
            { name: '🌐 Session', value: trade.session || 'N/A', inline: true },
            { name: '📈 Pair', value: (trade.pair || 'N/A').toUpperCase(), inline: true },
            { name: '⚖️ R:R', value: `\`${trade.rr || 'N/A'}\``, inline: true },
        ...
        if (trade.screenshot) embed.setImage(trade.screenshot);
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`journal_totw_approve_${interaction.user.id}`).setLabel('✅ Approve').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`journal_totw_deny_${interaction.user.id}`).setLabel('❌ Deny').setStyle(ButtonStyle.Danger),
        );
        await adminChannel.send({ embeds: [embed], components: [row] });
      }
      await interaction.editReply({ content: `✅ Trade #${tradeIdx + 1} submitted for admin review!`, components: [] });
    } catch (err) {
      if (err.code === 10062 || err.rawError?.message?.includes('already been acknowledged')) return;
      await interaction.editReply({ content: '❌ Error submitting.', components: [] }).catch(() => {});
    }
    return;
  }

  if (interaction.isModalSubmit() && interaction.customId === 'journal_totw_modal') {
    await interaction.deferReply({ ephemeral: true });
    try {
      const pair = interaction.fields.getTextInputValue('totw_pair');
      const pnl  = interaction.fields.getTextInputValue('totw_pnl');
      const rr   = interaction.fields.getTextInputValue('totw_rr');
      const proof = interaction.fields.getTextInputValue('totw_proof');
      const adminChannel = interaction.guild.channels.cache.get(process.env.ADMIN_CHANNEL_ID);
      if (adminChannel) {
        const embed = new EmbedBuilder()
          .setColor(0xF5F0E8)
          .setTitle('🏆 Trade of the Week Submission')
          .setDescription(`**${interaction.user.username}** submitted a Trade of the Week — ✍️ *manual entry*.`)
          .addFields(
            { name: '👤 User', value: `${interaction.user}`, inline: true },
            { name: '📈 Pair', value: pair.toUpperCase(), inline: true },
            { name: '💰 PnL', value: `**${pnl}**`, inline: true },
            ...(rr ? [{ name: '⚖️ R:R', value: `\`${rr}\``, inline: true }] : []),
            { name: '📎 Proof / Summary', value: proof, inline: false },
          )
          .setThumbnail(interaction.user.displayAvatarURL({ extension: 'png' }))
          .setFooter({ text: 'Elevate 🪽 • Approve or Deny' })
          .setTimestamp();
  ...
  if (interaction.isButton() && interaction.customId.startsWith('journal_totw_approve_')) {
    if (!interaction.member.permissions.has('Administrator')) { await interaction.reply({ content: '❌ Admins only.', ephemeral: true }); return; }
    const targetId = interaction.customId.replace('journal_totw_approve_', '');
    await interaction.deferUpdate();
    try {
      const db = loadDB();
      ...
      try { await targetMember?.send('✅ Your **Trade of the Week** was approved! +200 XP added. 🪽'); } catch {}
    } catch (err) {
      if (err.code === 10062 || err.rawError?.message?.includes('already been acknowledged')) return;
  ...
  if (interaction.isButton() && interaction.customId.startsWith('journal_totw_deny_')) {
    if (!interaction.member.permissions.has('Administrator')) { await interaction.reply({ content: '❌ Admins only.', ephemeral: true }); return; }
    const targetId = interaction.customId.replace('journal_totw_deny_', '');
    await interaction.deferUpdate();
    try {
      const targetMember = await interaction.guild.members.fetch(targetId).catch(() => null);
      await interaction.editReply({ content: `❌ Denied ${targetMember || targetId}.`, components: [] });
      try { await targetMember?.send('❌ Your **Trade of the Week** submission was denied. Please include valid proof.'); } catch {}
    } catch (err) {
      if (err.code === 10062 || err.rawError?.message?.includes('already been acknowledged')) return;
    }
    return;
