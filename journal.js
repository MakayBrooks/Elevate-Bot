...
function loadDB() { return _cache || {}; }
function saveDB(data) {
  _cache = data;
  dbStore.save(data).catch(e => console.error('Journal DB save error:', e));
}
async function initDB() {
  _cache = await dbStore.load();
  return _cache;
}
...
async function sendJournalPanel(forumChannel) {
  ...
  // Lock the thread so no one can reply to it
