...
const { postTicketCard, markTicketUpdated, runTicketCatchup, closeAllOpenTickets } = require('./tickets');
...
// Member count category auto-update
// Category ID: 1510789973656211566 (or set MEMBER_COUNT_CATEGORY_ID env var)
const MEMBER_COUNT_CATEGORY_ID = process.env.MEMBER_COUNT_CATEGORY_ID || '1510789973656211566';
let memberCountTimeout = null;

async function updateMemberCountCategory(guild) {
  try {
    await guild.members.fetch(); // ensure cache is fresh
    const category = guild.channels.cache.get(MEMBER_COUNT_CATEGORY_ID);
    if (!category) {
      // fallback: find by name pattern
      const found = guild.channels.cache.find(ch => /member.?count/i.test(ch.name) && ch.type === 4);
      if (!found) return;
      return updateMemberCountCategory._doRename(found, guild.memberCount);
    }
    return updateMemberCountCategory._doRename(category, guild.memberCount);
  } catch(err) { console.error('Member count update error:', err); }
}
updateMemberCountCategory._doRename = async function(category, count) {
  const newName = `👥Member Count: ${count}`;
  if (category.name === newName) return;
  try {
    await category.setName(newName);
    console.log(`✅ Member count category updated: ${count}`);
  } catch(err) { console.error('❌ Member count rename error:', err.message); }
};

function debouncedMemberCountUpdate(guild) {
  if (memberCountTimeout) clearTimeout(memberCountTimeout);
  // Debounce by 10s to avoid Discord rate limits (2 edits per 10 min)
  memberCountTimeout = setTimeout(() => updateMemberCountCategory(guild), 10000);
}
...
// Member count category — update on startup
try {
if (guild) await updateMemberCountCategory(guild);
} catch(err) { console.error('❌ Member count startup error:', err); }
...
// Close all open ticket forum threads (runs once, flagged in DB after)
try {
if (guild) {
await closeAllOpenTickets(guild);
}
} catch (err) { console.error('❌ Close tickets error:', err); }
...
// Member count — refresh every hour as a safety net
cron.schedule('0 * * * *', async () => {
if (guild) await updateMemberCountCategory(guild).catch(() => {});
});
...
// Update member count category
debouncedMemberCountUpdate(member.guild);
});

// Member leave — update member count
