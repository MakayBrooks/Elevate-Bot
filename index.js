  ...
  try {
    if (guild) {
      ...
      if (levelsChannel) {
        ...
        // Check if panel message still exists
        let panelExists = false;
        if (db.levelsPanelMessageId) {
          try {
            await levelsChannel.messages.fetch(db.levelsPanelMessageId);
            panelExists = true;
          } catch {}
        }
        if (!panelExists) {
  ...
  try {
    if (guild) {
      ...
      if (shopChannel) {
        ...
        if (db.shopMessageId) {
          try {
            await shopChannel.messages.fetch(db.shopMessageId);
            shopExists = true;
          } catch {}
        }
        if (!shopExists) {
