...
client.once('ready', async () => {
  ...
  try {
    ...
    if (guild) {
      ...
      if (journalChannel) {
        // Check if panel already exists so we don't double-post on every restart
        const pinned = await journalChannel.messages.fetchPinned();
        const alreadyPosted = pinned.some(m => m.author.id === client.user.id);
        if (!alreadyPosted) {
          const panel = await sendJournalPanel(journalChannel);
          await panel.pin();
          console.log('✅ Journal panel posted and pinned.');
        } else {
          console.log('📌 Journal panel already pinned, skipping.');
        }
      } else {
