...
const BUYABLE_BADGES = [
  { id: 'badge_rising',  name: '🌱 Rising Star', price: 2000,  boost: 0.10, description: '+10% XP boost on all earnings' },
  { id: 'badge_grinder', name: '⚡ Grinder',      price: 10000, boost: 0.20, description: '+20% XP boost on all earnings' },
  { id: 'badge_veteran', name: '🌟 Veteran',       price: 20000, boost: 0.30, description: '+30% XP boost on all earnings' },
];
...
async function postShopPanel(channel) {
  ...
  const badgeRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('shop_buy_badge_rising').setLabel('🌱 Rising Star — 2,000 pts').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('shop_buy_badge_grinder').setLabel('⚡ Grinder — 10,000 pts').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('shop_buy_badge_veteran').setLabel('🌟 Veteran — 20,000 pts').setStyle(ButtonStyle.Secondary),
  );
  ...
  setInterval(async () => {
    try {
      for (const [, guild] of client.guilds.cache) {
        // Fetch fresh membership + presence so we know who's actually online right now,
        // instead of trusting whatever happens to be cached.
        let members;
        ...
        for (const [uid, data] of Object.entries(db.users)) {
          ...
          if (!member) continue; // no longer in the server — don't pay them XP
          const status = member.presence?.status;
          // Only online/idle/dnd count as "present." Offline/invisible users were
          // previously earning XP forever just for having sent one message months ago.
