...
// Sync wrappers used throughout the file
...
function saveDB(data) {
  _cache = data;
  db.save(data).catch(e => console.error('DB save error:', e));
}

async function initDB() {
  _cache = await db.load();
  ...
  try {
    ...
    if (sorted.length === 0) {
      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        const badgeTags = [];
        if (data.equippedEarned) {
          const b = EARNED_BADGES.find(b => b.id === data.equippedEarned);
          if (b) badgeTags.push(`\`${b.name}\``);
        }
        if (data.equippedBought) {
          const b = BUYABLE_BADGES.find(b => b.id === data.equippedBought);
          if (b) badgeTags.push(`\`${b.name}\``);
        }
        ...
        const num = `**${i + 1}.**`;
        embed.addFields({
          name: `${medal}${num}  ${displayName}${badgeStr}`,
...
async function postShopPanel(channel) {
  const embed = new EmbedBuilder()
    ...
    .addFields(
      // Roles
      ...SHOP_ROLES.map(r => ({
        name: `${r.name} — ${r.price} pts`,
        value: r.id === 'role_gold' ? 'Gold member role' : r.id === 'role_platinum' ? 'Platinum member role' : 'Elite member role — highest tier',
        inline: true,
      })),
      { name: '\u200b', value: '\u200b', inline: false },
      // Buyable badges
      ...BUYABLE_BADGES.map(b => ({
        name: `${b.name} — ${b.price} pts`,
        value: b.description,
        inline: true,
      })),
      { name: '\u200b', value: '\u200b', inline: false },
  ...
  // Row 2: role buttons (blue = Primary)
  const roleRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('shop_buy_role_gold').setLabel('🌟 Elevate Gold — 500 pts').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('shop_buy_role_platinum').setLabel('💠 Elevate Platinum — 1000 pts').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('shop_buy_role_elite').setLabel('👑 Elevate Elite — 2000 pts').setStyle(ButtonStyle.Primary),
  );

  // Row 3: badge buttons (green = Success)
  const badgeRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('shop_buy_badge_rising').setLabel('🌱 Rising Star — 300 pts').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('shop_buy_badge_grinder').setLabel('⚡ Grinder — 600 pts').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('shop_buy_badge_veteran').setLabel('🌟 Veteran — 1000 pts').setStyle(ButtonStyle.Success),
  );
